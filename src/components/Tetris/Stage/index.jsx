      ...
      <Game portrait={portrait}>
        {nextRender && (
          <ContainerNext portrait={portrait} pixelSize={pixelSize}>
            ...
            <div className="display mt-5 flex justify-center bg-black  text-gray-200 font-bold px-10 py-5 border-white border-4">
              {`${Math.floor(time / 60)} : ${time % 60}`}
            </div>
            <div className="display-flex h-[300px] mt-10 overflow-x-hidden overflow-y-auto w-full">
              <h2 className=" text-gray-200 text-[30px] w-full">
                Recent Score
              </h2>
              <RecentScore
                level={Number(localStorage.getItem("gameLevel"))}
                simple={true}
              />
            </div>
          </ContainerNext>
        ...
        {status && (
          <ContainerStatus portrait={portrait} pixelSize={pixelSize}>
            <StatusRow
              ...
              padding={pixelSize / 2}
              title="GOAL TO WIN"
              value={localStorage.getItem("beatScore")}
            />
            <StatusRow
              backgroundColor={theme3d ? "#444" : "black"}
              portrait={portrait}
            ...
            <StatusRow
              ...
              title="LEVEL"
              value={
                Number(localStorage.getItem("gameLevel")) === 1
                  ? "Easy"
                  : Number(localStorage.getItem("gameLevel")) === 2
                  ? "Normal"
                  : "Hard"
              }
            />
